...
```mermaid
graph TD
    A["Decide mode: one-shot or interactive"] --> B["Decide mounts: workspace + isolated opencode config snapshot, plus skills/git-identity if needed"]
...
| `references/mounts-and-credentials.md`       | Credential redline, isolated config snapshot, standard mounts, and the `~/.claude` symlink gotcha |
