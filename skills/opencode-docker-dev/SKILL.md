...
Every run mounts the project directory (`/workspace`). When present,
`~/.config/opencode` is mounted **read-only** at
`/root/.config/opencode-host` and copied once at startup into an anonymous
writable volume at `/root/.config/opencode`. This keeps OpenCode's auth and
usage state inside the container; changes never flow back to the host. A
later `--exec-in` reuses that snapshot and does not refresh it. Add flags only
for what the task needs:
...
the default container image (`ghcr.io/luongnv89/devbox:latest-standard`);
...
- **Yes** → `docker rm -fv "$CONTAINER_NAME"`, then confirm `docker container inspect "$CONTAINER_NAME"` fails. The `-v` also removes the anonymous volume containing the copied config and credentials.
...
test — nothing else. Then ask the user whether to `docker rm -fv <name>`;
the `-v` removes the copied OpenCode config snapshot too.
...
the user declined to remove (leave it; do not `docker rm` on your own; use
`docker rm -fv` only after confirmation); a
...
··································································
  ...
  Image ready:           √ pass (ghcr.io/luongnv89/devbox:latest-standard)
  Mounts:                √ workspace, isolated opencode config snapshot [+ claude skills] [+ git identity]
