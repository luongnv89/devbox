...
```bash
cdev run --image u2204dev --workspace "$PWD"
...
- **Oh My Zsh** with custom configuration (`common/dev-image-base.sh:114`)
- **Starship** prompt for a fast, cross-shell experience (`common/dev-image-base.sh:123`)
...
- **Docker CLI** (client only) via `common/install-docker-cli.sh` (`common/dev-image-base.sh:59`). **kubectl** is not installed (`u2204dev/wedisagree.zsh-theme:129-130`).
...
Installed at image build time via [`common/install-ai-tools.sh`](../../common/install-ai-tools.sh) (same stack on all `*dev` images):
...
- **[asm](https://github.com/luongnv89/agent-skill-manager)** plus baked skills from [`luongnv89/idd`](https://github.com/luongnv89/idd) and [`luongnv89/skills`](https://github.com/luongnv89/skills)

CLIs install at npm `@latest` at image build. In a running container, run `update-ai-tools` (root or sudo) to upgrade the same set.

Inside the container: `opencode --version`, `pi --version`. With `cdev` mounts: `/workspace`, `/root/.ssh` (ro), `/root/.config/opencode`, `/root/.pi`, `/root/.codex`, `/root/.claude`.
...
```bash
# Build locally
docker build -t my-u2204dev -f u2204dev/Dockerfile .
